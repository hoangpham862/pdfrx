...
/// Simple class to hold target size, replaces Dart 3 Record type.
class TargetSize {
...
typedef CalculateTargetSize = TargetSize Function(int originalWidth, int originalHeight);
...
Future<JpegData> compressImageToJpeg(
  ...
  calculateTargetSize ??= (w, h) => TargetSize(w, h);
...
Future<Uint8List> _jpegEncodeAsync(Uint8List rgba, int width, int height, int quality) async {
  return await compute((jpegParams) {
    final Uint8List paramRgba = jpegParams[0] as Uint8List;
    final int paramWidth = jpegParams[1] as int;
    final int paramHeight = jpegParams[2] as int;
    final int paramQuality = jpegParams[3] as int;
    return JpegEncoder().compress(paramRgba, paramWidth, paramHeight, paramQuality);
  }, <dynamic>[rgba, width, height, quality]);
}
