...
//
class OffsetPdfHitResult {
  OffsetPdfHitResult({required this.offset, required this.hitResult});
  final Offset offset;
  final PdfPageHitTestResult hitResult;
}
...
class _PdfViewerState extends State<PdfViewer>
                      ...
                      iv.InteractiveViewer(
                        ...
                        boundaryMargin: widget.params.scrollPhysics == null
                            ...
                            : widget.params.pageTransition == PageTransition.discrete
                                ? EdgeInsets.zero // Discrete mode uses boundaryProvider
                                : _adjustedBoundaryMargins,
                        boundaryProvider:
                            widget.params.pageTransition == PageTransition.discrete ? _getDiscreteBoundaryRect : null,
                        ...
                        child: GestureDetector(
                          ...
                          onDoubleTapDown: (d) => _handleGeneralTap(d.globalPosition, PdfViewerGeneralTapType.doubleTap),
                          onLongPressStart: (d) => _handleGeneralTap(d.globalPosition, PdfViewerGeneralTapType.longPress),
                          onSecondaryTapUp: (d) => _handleGeneralTap(d.globalPosition, PdfViewerGeneralTapType.secondaryTap),
  ...
  Offset _calcOverscroll(Matrix4 m, {required Size viewSize, bool allowExtendedBoundaries = true}) {
    ...
    final rightBoundary = layout.documentSize.width + boundaryMargin.right; // negative margin reduces allowed rightward scroll
    final topBoundary = -boundaryMargin.top; // negative margin reduces allowed upward scroll
    final bottomBoundary = layout.documentSize.height + boundaryMargin.bottom; // negative margin reduces allowed downward scroll
  ...
  Matrix4 _calcMatrixForClampedToNearestBoundary(Matrix4 candidate, {required Size viewSize}) {
    ...
    return candidate.clone()..translate(-overScroll.dx, -overScroll.dy);
  }
  ...
  void _updateLayout(Size viewSize) {
    ...
    if (!_initialized && _layout != null && _fitScale != _defaultMinScale) {
      ...
      Future.microtask(() async {
        ...
        _calcFitScale();
        _calcZoomStopTable();
        final zoom = widget.params.calculateInitialZoom?.call(
              _document!,
              _controller!,
      ...
      if (isLayoutChanged) {
        ...
        if (zoomTo != _currentZoom) {
          ...
          final pivotScale = Matrix4.identity()
            ...
            ..scale(zoomChange)
            ..translate(-pivot.x / zoomChange, -pivot.y / zoomChange);
  ...
  OffsetPdfHitResult? _nudgeHitTest(Offset start, {PdfPageLayout? layout, int? pageIndex}) {
    const epsViewPx = 1.0;
    ...
    if (initialResult != null) {
      return OffsetPdfHitResult(offset: Offset.zero, hitResult: initialResult);
    }
    ...
    if (result != null) {
      return OffsetPdfHitResult(offset: nudgeOffset, hitResult: result);
    }
  ...
  Future<void> _onInteractionEnd(ScaleEndDetails details) async {
    ...
    final shouldHandleDiscrete =
        (widget.params.pageTransition == PageTransition.discrete && (!_hadScaleChangeInInteraction && !_hasActiveAnimations));
  ...
  Future<void> _handleDiscretePageTransition(ScaleEndDetails details) async {
    ...
    final scrollVelocity = isPrimaryVertical ? details.velocity.pixelsPerSecond.dy : details.velocity.pixelsPerSecond.dx;
    final crossAxisVelocity = isPrimaryVertical ? details.velocity.pixelsPerSecond.dx : details.velocity.pixelsPerSecond.dy;
    ...
    if (hasSignificantVelocity) {
      ...
      final velocityDirection = scrollVelocity > 0 ? -1 : 1; // INVERTED: positive velocity = viewport moves up (previous page)
  ...
  bool _isAtBoundary(Rect visibleRect, int pageNumber, PdfPageLayout layout, bool isPrimaryVertical, int direction) {
    ...
    if (isMovingForward) {
      ...
      return isPrimaryVertical ? visibleRect.top <= pageRect.top + tolerance : visibleRect.left <= pageRect.left + tolerance;
    }
  }
  ...
  PdfPageAnchor _getSnapAnchor({
    ...
    if (targetPageNumber == currentPageNumber) {
      ...
      final pageCenter = isPrimaryVertical ? (pageRect.top + pageRect.bottom) / 2 : (pageRect.left + pageRect.right) / 2;
  ...
  int? _guessCurrentPageNumber() {
    ...
    final layout = _layout!;
    final isHorizontal = layout.primaryAxis == Axis.horizontal || layout.documentSize.width > layout.documentSize.height;
    ...
    if (_usingScrollPercentageMode) {
      ...
      final scrollLength = (isHorizontal ? layout.documentSize.width : layout.documentSize.height) -
  ...
  PdfPageLayout _addDiscreteSpacing(PdfPageLayout layout, PdfLayoutHelper helper) {
    ...
    if (layout is PdfSpreadLayout) {
      ...
      for (var pageNum = 1; pageNum <= layout.pageLayouts.length; pageNum++) {
        ...
        final crossAxisSpreadSizeWithMargins = isPrimaryVertical ? scaledSpreadWidthWithMargins : scaledSpreadHeightWithMargins;
        ...
        for (var p = range.firstPageNumber; p <= range.lastPageNumber; p++) {
          ...
          final pageOffsetInSpreadPrimary =
              isPrimaryVertical ? pageRect.top - spreadBounds.top : pageRect.left - spreadBounds.left;
          final pageOffsetInSpreadCross = isPrimaryVertical ? pageRect.left - spreadBounds.left : pageRect.top - spreadBounds.top;
          ...
          final newRect = isPrimaryVertical
              ? Rect.fromLTWH(
                  crossAxisSpreadContentStart + scaledPageOffsetInSpreadCross, // Horizontal: relative to centered spread content
                  spreadContentStart + scaledPageOffsetInSpreadPrimary, // Vertical: relative to centered spread content
                  scaledPageWidth,
                  scaledPageHeight,
                )
              : Rect.fromLTWH(
                  spreadContentStart + scaledPageOffsetInSpreadPrimary, // Horizontal: relative to centered spread content
                  crossAxisSpreadContentStart + scaledPageOffsetInSpreadCross, // Vertical: relative to centered spread content
                  scaledPageWidth,
                  scaledPageHeight,
                );
      ...
      for (var i = 0; i < layout.pageLayouts.length; i++) {
        ...
        final crossAxisPageSizeWithMargins = isPrimaryVertical ? scaledPageWidthWithMargins : scaledPageHeightWithMargins;
        final crossAxisViewport = isPrimaryVertical ? helper.viewWidth : helper.viewHeight;
  ...
  Rect? _getDiscreteBoundaryRect(Rect visibleRect, Size childSize, {double? zoom}) {
    ...
    if (layout == null || currentPageNumber == null || currentPageNumber < 1 || currentPageNumber > layout.pageLayouts.length) {
      return null;
    }
    ...
    if (!userBoundaryMargin.containsInfinite && _viewSize != null) {
      ...
      final needsPrimaryAxisMargin = scaledPrimaryAxisSize >= primaryAxisViewport ||
          (isPrimaryVertical
              ? (userBoundaryMargin.top < 0 || userBoundaryMargin.bottom < 0)
              : (userBoundaryMargin.left < 0 || userBoundaryMargin.right < 0));
      final needsCrossAxisMargin = scaledCrossAxisSize >= crossAxisViewport ||
          (isPrimaryVertical
              ? (userBoundaryMargin.left < 0 || userBoundaryMargin.right < 0)
              : (userBoundaryMargin.top < 0 || userBoundaryMargin.bottom < 0));
    ...
    final shouldExtendBoundaries = _isActiveGesture && !_hadScaleChangeInInteraction && !_isActivelyZooming && _viewSize != null;
  ...
  void _paintPagesCustom(
    ...
    for (var i = 0; i < _document!.pages.length; i++) {
      ...
      final getPageRenderingScale = widget.params.getPageRenderingScale ??
  ...
  Matrix4 _calcMatrixForArea({required Rect rect, double? zoomMax, double? margin, PdfPageAnchor? anchor}) => _calcMatrixForRect(
  ...
  Future<void> _goToPosition({
    ...
    final marginAdjusted =
        pageNumber != null ? _calcMatrixForMarginSnappedToNearestBoundary(m, pageNumber: pageNumber, viewSize: _viewSize!) : m;
  ...
  Offset _documentToLocal(Offset document) {
    return document.scale(_currentZoom, _currentZoom).translate(_txController.value.xZoomed, _txController.value.yZoomed);
  }
  ...
  List<Widget> _placeTextSelectionWidgets(BuildContext context, Size viewSize, bool isCopyTextEnabled) {
    ...
    if ((enableSelectionHandles || _selectionPointerDeviceKind == PointerDeviceKind.touch) &&
      ...
      if (_textSelA != null) {
        ...
        final offset =
            widget.params.textSelectionParams?.calcSelectionHandleOffset?.call(context, _textSelA!, state) ?? Offset.zero;
      ...
      if (_textSelB != null) {
        final state = _selPartMoving == _TextSelectionPart.b
            ...
            : _hoverOn == _TextSelectionPart.b
                ...
                : PdfViewerTextSelectionAnchorHandleState.normal;
        final offset =
            widget.params.textSelectionParams?.calcSelectionHandleOffset?.call(context, _textSelB!, state) ?? Offset.zero;
    ...
    final _TextSelectionPart textAnchorMoving;
    switch (_selPartMoving) {
      case _TextSelectionPart.a:
        textAnchorMoving = _selA! < _selB! ? _TextSelectionPart.a : _TextSelectionPart.b;
        break;
      case _TextSelectionPart.b:
        textAnchorMoving = _selA! < _selB! ? _TextSelectionPart.b : _TextSelectionPart.a;
        break;
      default:
        textAnchorMoving = _selPartMoving;
    }
    ...
    if (((textAnchorMoving == _TextSelectionPart.a || textAnchorMoving == _TextSelectionPart.b) &&
      ...
      final magnifierEnabled = (magnifierParams.enabled ?? _selectionPointerDeviceKind == PointerDeviceKind.touch) &&
      ...
      if (magnifierEnabled) {
        ...
        final magnifierPosition = (magnifierParams.calcPosition ?? calcPosition)(
              _magnifierRect?.size,
              anchorLocalRect,
              handleLocalRect,
              textAnchor,
    ...
    final showContextMenuAutomatically =
        widget.params.textSelectionParams?.showContextMenuAutomatically ?? _selectionPointerDeviceKind == PointerDeviceKind.touch;
    ...
    if (showContextMenu &&
        ...
        (_contextMenuDocumentPosition != null ||
            ...
            _isSelectingAllText) &&
        isCopyTextEnabled) {
      final localOffset = _contextMenuDocumentPosition != null ? offsetToLocal(context, _contextMenuDocumentPosition!) : null;
      ...
      if (contextMenu != null && !isPositionalWidget(contextMenu)) {
        ...
        final offset = localOffset != null
            ...
            : (calcPosition(_contextMenuRect?.size, anchorLocalRect, handleLocalRect, textAnchor, _pointerOffset) ?? Offset.zero);
  ...
  Widget _buildHandle(BuildContext context, Path path, PdfViewerTextSelectionAnchorHandleState state) {
    final baseColor = Theme.of(context).textSelectionTheme.selectionColor ?? DefaultSelectionStyle.of(context).selectionColor!;
    ...
    // }
    var shadow = false;
    var selectionColor = baseColor;
    if (state == PdfViewerTextSelectionAnchorHandleState.normal) {
      selectionColor = baseColor.withAlpha(70);
      shadow = true;
    } else if (state == PdfViewerTextSelectionAnchorHandleState.dragging ||
        state == PdfViewerTextSelectionAnchorHandleState.hover) {
  ...
  Rect _getMagnifierRect(
    ...
    if ((_document!.pages[textAnchor.page.pageNumber - 1].rotation.index & 1) == 0) {
      width = c.width;
      height = c.height;
    } else {
      width = c.height;
          ...
          child: CustomPaint(
            painter: _CustomPainter.fromFunctions((canvas, size) {
              ...
              _paintPagesCustom(
                ...
                maxImageCacheBytes: widget.params.textSelectionParams?.magnifier?.maxImageBytesCachedOnMemory ??
  ...
  Future<void> selectWord(Offset offset, {PointerDeviceKind? deviceKind}) async {
    for (var i = 0; i < _document!.pages.length; i++) {
      ...
      final page = _document!.pages[i];
      final point = offset.translate(-pageRect.left, -pageRect.top).toPdfPoint(page: page, scaledPageSize: pageRect.size);
...
class _PdfPageImageCache {
  ...
  void removeCacheImagesIfCacheBytesExceedsLimit(
    ...
    var bytesConsumed = pageImages.values.fold(0, (sum, e) => sum + getBytesConsumed(e.image)) +
...
class PdfTextSelectionAnchor {
  ...
  bool operator ==(Object other) {
    ...
    return rect == other.rect && direction == other.direction && type == other.type && page == other.page && index == other.index;
  }
...
class PdfViewerController extends ValueListenable<Matrix4> {
  ...
  Matrix4? calcMatrixForFit({required int pageNumber}) => calcMatrixForDest(PdfDest(pageNumber, PdfDestCommand.fit, null));
  ...
  /// [anchor] specifies how the page is positioned if the page is larger than the view.
  Matrix4 calcMatrixForArea({required Rect rect, PdfPageAnchor? anchor}) => _state._calcMatrixForArea(rect: rect, anchor: anchor);
