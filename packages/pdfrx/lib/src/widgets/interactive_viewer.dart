...
class InteractiveViewerState extends State<InteractiveViewer> with TickerProviderStateMixin {
  ...
  Matrix4 _matrixTranslate(Matrix4 matrix, Offset translation) {
    ...
    final alignedTranslationMap = {
      PanAxis.horizontal: translation,
      PanAxis.vertical: Offset(0, translation.dy),
    ...
    if (_currentAxis != null && _gestureType == _GestureType.pan) {
      alignedTranslation = alignedTranslationMap[widget.panAxis]!;
    } else {
      alignedTranslation = translation;
    }

    final nextMatrix = matrix.clone()..translate(alignedTranslation.dx, alignedTranslation.dy);
    ...
    if (widget.scrollPhysics != null) {
      final physics =
          (_gestureType == _GestureType.scale) ? (widget.scrollPhysicsScale ?? widget.scrollPhysics!) : widget.scrollPhysics!;
      ...
      final overscrollX = proposedX == currentOffset.dx ? 0 : physics.applyBoundaryConditions(metricsX, proposedX); // : 0.
      final overscrollY = proposedY == currentOffset.dy ? 0 : physics.applyBoundaryConditions(metricsY, proposedY); // : 0.
  ...
  Matrix4 _matrixScale(Matrix4 matrix, double scale) {
    ...
    if (scrollPhysics != null) {
      ...
      if (_gestureType == _GestureType.scale &&
        ...
        return matrix.clone()..scale(factor, factor, factor);
      } else {
    ...
    switch (_gestureType) {
      case _GestureType.pan:
        if (widget.scrollPhysics != null) {
          ...
          final flingVelocityX = math.min(details.velocity.pixelsPerSecond.dx.abs(), widget.scrollPhysics!.maxFlingVelocity) *
              details.velocity.pixelsPerSecond.dx.sign;
          final flingVelocityY = math.min(details.velocity.pixelsPerSecond.dy.abs(), widget.scrollPhysics!.maxFlingVelocity) *
      ...
      case _GestureType.scale:
        if (widget.scrollPhysics != null) {
          ...
          final endMatrix = _snapStartMatrix.clone()
            ...
            ..scale(clampedScale / endScale, clampedScale / endScale)
            ..translate(-pivotScene.dx, -pivotScene.dy);
  ...
  Rect _computePanBoundaries({
    ...
    final baseMargin = (overrideAutoAdjustBoundaries && !widget.scrollPhysicsAutoAdjustBoundaries) || boundaryMargin == null
  ...
  bool get hasActiveAnimations => _controller.isAnimating || _scaleController.isAnimating || _snapController.isAnimating;
