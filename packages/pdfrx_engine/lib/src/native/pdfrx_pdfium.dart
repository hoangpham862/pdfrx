...
Future<void> _init() async {
  ...
  await _initSync.synchronized(() async {
    ...
    BackgroundWorker.computeWithArena((arena, params) {
      // Ép kiểu params sang Map để có thể dùng cú pháp ['key']
      final paramsMap = params as Map<String, dynamic>;

      final config = arena<pdfium_bindings.FPDF_LIBRARY_CONFIG>();
      config.ref.version = 2;

      // Lấy đường dẫn font từ Map
      final fontPaths = <String>[];
      final appLocalFontPath = paramsMap['appLocalFontPath'] as Directory?;
      if (appLocalFontPath != null) {
        fontPaths.add(appLocalFontPath.path);
      }

      final extraFontPaths = paramsMap['fontPaths'] as List<String>;
      fontPaths.addAll(extraFontPaths);
      ...
      'appLocalFontPath': _appLocalFontPath,
      'fontPaths': Pdfrx.fontPaths,
    });
    ...
    return _openByFunc(
      (password) async => BackgroundWorker.computeWithArena((arena, params) {
        ...
        final doc = pdfium.FPDF_LoadDocument(
          (paramsMap['filePath'] as String).toUtf8(arena),
          (paramsMap['password'] as String?)?.toUtf8(arena) ?? nullptr,
        );
        return doc.address;
      }, {
        'filePath': filePath,
      ...
      try {
        ...
        return _openByFunc(
          (password) async => BackgroundWorker.computeWithArena(
            (arena, params) {
              final paramsMap = params as Map<String, dynamic>;
              return pdfium.FPDF_LoadMemDocument(
                Pointer<Void>.fromAddress(paramsMap['buffer'] as int),
                paramsMap['fileSize'] as int,
                (paramsMap['password'] as String?)?.toUtf8(arena) ?? nullptr,
              ).address;
            },
            {
        ...
        (password) async => BackgroundWorker.computeWithArena(
          (arena, params) {
            ...
            return pdfium.FPDF_LoadCustomDocument(
              Pointer<pdfium_bindings.FPDF_FILEACCESS>.fromAddress(paramsMap['fileAccess'] as int),
              (paramsMap['password'] as String?)?.toUtf8(arena) ?? nullptr,
            ).address;
          },
          {
            'fileAccess': fa.fileAccess,
            'password': password,
          },
        ),
        sourceName: sourceName,
    ...
    try {
      ...
      final doc = await BackgroundWorker.computeWithArena(
        (arena, params) {
          ...
          final newPage = pdfium.FPDFPage_New(
            document,
            0,
            paramsMap['width'] as double,
            paramsMap['height'] as double,
          );
...
class _PdfDocumentPdfium extends PdfDocument {
  ...
  static Future<PdfDocument> fromPdfDocument(
    ...
    try {
      ...
      pdfDoc = _PdfDocumentPdfium._(
        doc,
        sourceName: sourceName,
        securityHandlerRevision: resultMap['securityHandlerRevision'] as int,
        permissions: (resultMap['securityHandlerRevision'] as int) != -1
            ? PdfPermissions(resultMap['permissions'] as int, resultMap['securityHandlerRevision'] as int)
            : null,
        formHandle: pdfium_bindings.FPDF_FORMHANDLE.fromAddress(resultMap['formHandle'] as int),
        formInfo: Pointer<pdfium_bindings.FPDF_FORMFILLINFO>.fromAddress(resultMap['formInfo'] as int),
        disposeCallback: disposeCallback,
      );
      ...
      pdfDoc._pages = List.unmodifiable(pages['pages'] as List<PdfPage>);
  ...
  Future<void> loadPagesProgressively<T>({
    ...
    for (;;) {
      ...
      if (isDisposed) return;
      pages = loaded['pages'] as List<PdfPage>;

      if (onPageLoadProgress != null) {
        final result = await onPageLoadProgress(loaded['pageCountLoadedTotal'] as int, (loaded['pages'] as List).length, data);
        if (result == false) {
          // If the callback returns false, stop loading pages
          return;
        }
      }
      if ((loaded['pageCountLoadedTotal'] as int) == (loaded['pages'] as List).length) {
  ...
  Future<Map<String, dynamic>> _loadPagesInLimitedTime({
    ...
    try {
      final results = await BackgroundWorker.computeWithArena(
        (arena, params) {
          final paramsMap = params as Map<String, dynamic>;
          final doc = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['docAddress'] as int);
          final pageCount = pdfium.FPDF_GetPageCount(doc);
          final end = paramsMap['maxPageCountToLoadAdditionally'] == null
              ? pageCount
              : min(
                  pageCount, (paramsMap['pagesCountLoadedSoFar'] as int) + (paramsMap['maxPageCountToLoadAdditionally'] as int));
          final t = paramsMap['timeoutUs'] != null ? (Stopwatch()..start()) : null;
          final pages = <Map<String, dynamic>>[];
          for (var i = paramsMap['pagesCountLoadedSoFar'] as int; i < end; i++) {
            ...
            if (t != null && t.elapsedMicroseconds > (paramsMap['timeoutUs'] as int)) {
              break;
            }
          }
          return {
      ...
      final resultsPages = List<Map<String, dynamic>>.from(results['pages'] as List);
      for (var i = 0; i < resultsPages.length; i++) {
        final pageData = resultsPages[i];
        pages.add(
          _PdfPagePdfium._(
            document: this,
            pageNumber: pages.length + 1,
            width: pageData['width'] as double,
            height: pageData['height'] as double,
            rotation: PdfPageRotation.values[pageData['rotation'] as int],
            bbLeft: pageData['bbLeft'] as double,
            bbBottom: pageData['bbBottom'] as double,
            isLoaded: true,
          ),
        );
      }
      ...
      if (pageCountLoadedTotal > 0) {
        ...
        for (var i = pages.length; i < (results['totalPageCount'] as int); i++) {
          pages.add(
            _PdfPagePdfium._(
              document: this,
              pageNumber: pages.length + 1,
      ...
      return {
        'pages': pages,
        'pageCountLoadedTotal': pageCountLoadedTotal,
      };
    } catch (e) {
      rethrow;
    }
  ...
  Future<void> reloadPages({List<int>? pageNumbersToReload}) async {
    try {
      final results = await BackgroundWorker.computeWithArena((arena, params) {
        ...
        final doc = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['docAddress'] as int);
        final pageCount = pdfium.FPDF_GetPageCount(doc);
        if (paramsMap['pageNumbersToReload'] != null) {
          for (final pageNumber in (paramsMap['pageNumbersToReload'] as List<int>)) {
        ...
        final reloadList = (paramsMap['pageNumbersToReload'] as List<int>?) ?? <int>[];
        final pageNumbersToLoad = SplayTreeSet<int>.from(reloadList);
        pageNumbersToLoad.addAll(
          Iterable.generate(
            pageCount - (paramsMap['currentPageCount'] as int),
            (index) => (paramsMap['currentPageCount'] as int) + index + 1,
          ),
        );
        ...
        'docAddress': document.address,
        'pageNumbersToReload': pageNumbersToReload,
        'currentPageCount': _pages.length,
      });
      ...
      for (var i = 0; i < resultsPages.length; i++) {
        final pageData = resultsPages[i];
        final pageIndex = pageData['pageIndex'] as int;
  ...
  @override
  bool isIdenticalDocumentHandle(Object? other) => other is _PdfDocumentPdfium && document.address == other.document.address;
  ...
  Future<void> dispose() async {
    if (!isDisposed) {
      ...
      await BackgroundWorker.compute((params) {
        final paramsMap = params as Map<String, dynamic>;
        final formHandle = pdfium_bindings.FPDF_FORMHANDLE.fromAddress(paramsMap['formHandle'] as int);
        final formInfo = Pointer<pdfium_bindings.FPDF_FORMFILLINFO>.fromAddress(paramsMap['formInfo'] as int);
        pdfium.FPDFDOC_ExitFormFillEnvironment(formHandle);
        calloc.free(formInfo);

        final doc = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['document'] as int);
  ...
  Future<List<PdfOutlineNode>> loadOutline() async => isDisposed
      ? <PdfOutlineNode>[]
      : await BackgroundWorker.computeWithArena((arena, params) {
          final document = pdfium_bindings.FPDF_DOCUMENT.fromAddress(params['document'] as int);
  ...
  Future<Uint8List> encodePdf({bool incremental = false, bool removeSecurity = false}) async {
    ...
    return await BackgroundWorker.computeWithArena((arena, params) {
      ...
      try {
        final paramsMap = params as Map<String, dynamic>;
        final document = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['document'] as int);
        final fw = arena<pdfium_bindings.FPDF_FILEWRITE>();
        fw.ref.version = 1;
        fw.ref.WriteBlock = nativeWriteCallable.nativeFunction;
        final int flags;
        if (paramsMap['removeSecurity'] as bool) {
          flags = 3; // FPDF_SAVE_NO_SECURITY(3)
        } else {
          flags = paramsMap['incremental'] ? 1 : 2; // FPDF_INCREMENTAL(1) or FPDF_NO_INCREMENTAL(2)
        }
...
class _DocumentPageArranger with ShuffleItemsInPlaceMixin {
  ...
  static Future<bool> doShufflePagesInPlace(_PdfDocumentPdfium document) async {
    final indices = <int>[];
    final rotations = <int?>[];
    final items = <int, Map<String, int>>{};
    var modifiedCount = 0;
    ...
    await BackgroundWorker.computeWithArena(
      (arena, params) {
        ...
        final arranger = _DocumentPageArranger._(
          pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['document'] as int),
          Map<int, Map<String, int>>.from(paramsMap['items'] as Map),
        );
        arranger.shuffleInPlaceAccordingToIndices(paramsMap['indices'] as List<int>);

        for (var i = 0; i < (paramsMap['length'] as int); i++) {
          final rotation = (paramsMap['rotations'] as List)[i] as int?;
  ...
  final pdfium_bindings.FPDF_DOCUMENT document;
  final Map<int, Map<String, int>> items;
...
class _PdfPagePdfium extends PdfPage {
    ...
    try {
      ...
      final isSucceeded = await using((arena) async {
        ...
        return await BackgroundWorker.compute(
          (params) {
            final paramsMap = params as Map<String, dynamic>;
            final cancelFlag = Pointer<Bool>.fromAddress(paramsMap['cancelFlag'] as int);
            if (cancelFlag.value) return false;
            final bmp = pdfium.FPDFBitmap_CreateEx(
              paramsMap['width'] as int,
              paramsMap['height'] as int,
              pdfium_bindings.FPDFBitmap_BGRA,
              Pointer.fromAddress(paramsMap['buffer'] as int),
              (paramsMap['width'] as int) * rgbaSize,
            );
            if (bmp == nullptr) {
              throw PdfException('FPDFBitmap_CreateEx(${paramsMap['width']}, ${paramsMap['height']}) failed.');
            }
            pdfium_bindings.FPDF_PAGE page = nullptr;
            try {
              final doc = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['document'] as int);
              page = pdfium.FPDF_LoadPage(doc, (paramsMap['pageNumber'] as int) - 1);
              if (page == nullptr) {
                throw PdfException('FPDF_LoadPage(${paramsMap['pageNumber']}) failed.');
              }
              pdfium.FPDFBitmap_FillRect(
                  bmp, 0, 0, paramsMap['width'] as int, paramsMap['height'] as int, paramsMap['backgroundColor'] as int);
              ...
              pdfium.FPDF_RenderPageBitmap(
                ...
                -(paramsMap['x'] as int),
                -(paramsMap['y'] as int),
                paramsMap['fullWidth'] as int,
                paramsMap['fullHeight'] as int,
                paramsMap['rotation'] as int,
                (paramsMap['flags'] as int) |
                    ((paramsMap['annotationRenderingMode'] as PdfAnnotationRenderingMode) != PdfAnnotationRenderingMode.none
                        ? pdfium_bindings.FPDF_ANNOT
                        : 0),
              );

              if ((paramsMap['formHandle'] as int) != 0 &&
                  (paramsMap['annotationRenderingMode'] as PdfAnnotationRenderingMode) ==
                      PdfAnnotationRenderingMode.annotationAndForms) {
                pdfium.FPDF_FFLDraw(
                  pdfium_bindings.FPDF_FORMHANDLE.fromAddress(paramsMap['formHandle'] as int),
                  bmp,
                  page,
                  -(paramsMap['x'] as int),
                  -(paramsMap['y'] as int),
  ...
  Future<PdfPageRawText?> loadText() async {
    ...
    return await BackgroundWorker.computeWithArena((arena, params) {
      ...
      final doc = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['docHandle'] as int);
      final page = pdfium.FPDF_LoadPage(doc, (paramsMap['pageNumber'] as int) - 1);
      ...
      try {
        ...
        for (var i = 0; i < charCount; i++) {
          ...
          charRects.add(_rectFromLTRBBuffer(rectBuffer, paramsMap['bbLeft'] as double, paramsMap['bbBottom'] as double));
        }
  ...
  Future<List<PdfLink>> _loadWebLinks() async => document.isDisposed
      ...
      : await BackgroundWorker.computeWithArena((arena, params) {
          ...
          try {
            final document = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['document'] as int);
            page = pdfium.FPDF_LoadPage(document, (paramsMap['pageNumber'] as int) - 1);
            ...
            return List.generate(pdfium.FPDFLink_CountWebLinks(linkPage), (index) {
              final rects = List.generate(pdfium.FPDFLink_CountRects(linkPage, index), (rectIndex) {
                ...
                return _rectFromLTRBBuffer(rectBuffer, paramsMap['bbLeft'] as double, paramsMap['bbBottom'] as double);
              });
  ...
  Future<List<PdfLink>> _loadAnnotLinks() async => document.isDisposed
      ...
      : await BackgroundWorker.computeWithArena((arena, params) {
          final paramsMap = params as Map<String, dynamic>;
          final document = pdfium_bindings.FPDF_DOCUMENT.fromAddress(paramsMap['document'] as int);
          final page = pdfium.FPDF_LoadPage(document, (paramsMap['pageNumber'] as int) - 1);
          try {
            ...
            for (var i = 0; i < count; i++) {
              ...
              ).translate(-(paramsMap['bbLeft'] as double), -(paramsMap['bbBottom'] as double));
...
typedef _NativeFileReadCallable
    = NativeCallable<Int Function(Pointer<Void> param, UnsignedLong position, Pointer<UnsignedChar> pBuf, UnsignedLong size)>;
